...
use crate::utils::{get_app_cache_directory, pack_metadata_as_struct, MetadataInfo};
...
pub async fn generate_thumbnail_image(
    ...
    match File::open(Path::new(&absolute_out_path)).await {
        Ok(_) => {
            return Ok((absolute_out_path, metadata));
        }
        Err(e) => match e.kind() {
